...
//   const DateSelector({super.key, required void Function(DateTime newDate) onDateSelected});
...
class DateSelector extends StatefulWidget {
  final Function(DateTime newDate) onDateSelected; // Corrected this line
...
class _DateSelectorState extends State<DateSelector> {
  ...
  // Generates the list of dates for the current week offset
  List<DateTime> generateWeekDates(int weekOffset) {
    final today = DateTime.now();
    DateTime startOfWeek = today.subtract(Duration(days: today.weekday - 1));
    startOfWeek = startOfWeek.add(Duration(days: weekOffset * 7));
